class GameOfLife {
  ...
  livingNeighbors(row, col) {
    ...
    for(let i=0;i<array.length;i++) {
      if((array[i][0] >= 0 && array[i][1] >= 0) && (array[i][0] < this.height && array[i][1] < this.width)) {
        if(board[array[i][0]][array[i][1]] !== 0) {
          count++;
        }
      }
    }
  ...
  tick() {
    ...
    for(let r=0;r<this.height;r++) {
      for(let c=0;c<this.width;c++) {
         if(this.livingNeighbors(r,c) < 2 && this.getCell(r,c) === 1) { // dies
            newBoard[r][c] = 0;
         }
         if((this.livingNeighbors(r,c) === 2 || this.livingNeighbors(r,c) === 3) && (this.getCell(r,c) === 1)) { // survives
            newBoard[r][c] = 1;
         }
         if(this.livingNeighbors(r,c) > 3 && this.getCell(r,c) === 1) { // dies
            newBoard[r][c] = 0;
         }
         if(this.livingNeighbors(r,c) === 3 && this.getCell(r,c) === 0) { // reproduces/comes back to life
            newBoard[r][c] = 1;
         }
      }
   }
